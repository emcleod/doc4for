...
class TestFunctionBindings(TestCase):
    ...
    """
        ...
        self.assertIn('binding_type', function)
        self.assertEqual(function['binding_type']['type'], BindingTypeEnum.DEFAULT)
        self.assertEqual(function['binding_type']['name'], None)
    ...
    module binding_names_mod
        ...
        !!* Different parameter order, compact *!
        function func4(x) bind(name="c_func4",c) result(y)
            use iso_c_binding
            real(c_double), intent(in) :: x
            real(c_double) :: y
        end function func4
